  ...
  public function procesando_usuarios(){
   ...
   // Creación de especialista o Administrador (Nuevo Colaborador)
	...
	if($this->session->userdata('session') === TRUE ){
		  ...
		  $data['secciones']   = $this->modelo->get_catalogo_secciones();
		  $data['grupos']   = $this->modelo->get_catalogo_grupos();
	...
	function validar_nuevo_usuario(){
		if ($this->session->userdata('session') !== TRUE) {
			...
			$this->form_validation->set_rules( 'matricula', 'Matricula', 'trim|required|min_length[3]|max_length[100]|xss_clean');
			//id_seccion, id_grupo
			...
			if ($this->form_validation->run() === TRUE){
				if ($this->input->post( 'pass_1' ) === $this->input->post( 'pass_2' ) ){
					...
					if ( $login_check != FALSE ){		
						...
						$usuario['id_grupo']   			= $this->input->post( 'id_grupo' );
						$usuario['id_seccion']   			= $this->input->post( 'id_seccion' );
	...
	if($this->session->userdata('session') === TRUE ){
		  ...
		  switch ($id_perfil) {    
			...
			case 3: //Profesores
			case 4: //Alumnos
				$this->load->view( 'principal/dashboard_otros',$data );
				break;
			case 5:
				$this->load->view( 'principal/dashboard_checador',$data );
			  break;
